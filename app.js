...
async function init() {
  ...
  const lessonId = params.get("lesson") || "rational-discrimination";
  ...
  // 모든 섹션 평탄화 (차시 간 이동을 위해)
  app.flatSections = [];
  app.lesson.sessions.forEach(session => {
    session.sections.forEach(sec => {
      app.flatSections.push({ ...sec, sessionTitle: session.title });
    });
  });
  ...
  const idx = app.flatSections.findIndex(s => s.id === hash);
  goTo(idx >= 0 ? idx : 0);

  bindKeyboard();
  window.addEventListener("hashchange", () => {
    const h = location.hash.replace("#", "");
    const i = app.flatSections.findIndex(s => s.id === h);
...
function renderSidebar() {
  document.getElementById("sidebar-title").textContent = app.lesson.title;
  document.getElementById("sidebar-subtitle").textContent = app.lesson.subtitle || "";
  ...
  app.lesson.sessions.forEach(session => {
    ...
    label.className = "sidebar__session";
...
/* ---------- 섹션 이동 ---------- */
function goTo(idx) {
  if (idx < 0 || idx >= app.flatSections.length) return;
  app.currentIdx = idx;
  const sec = app.flatSections[idx];
...
function renderSection(sec) {
  ...
  header.innerHTML = `
    <div class="section-header__id">${escapeHtml(sec.id)} · ${escapeHtml(sec.sessionTitle)}</div>
...
function renderQuestion(block) {
  ...
  block.prompts.forEach(pr => {
    ...
    // 질문에 딸린 작은 답 (사이드바 스타일 — 바로 보이게 할지 숨길지 결정 필요)
    // 1-3처럼 "답이 정답지에 가까운" 경우는 토글로
    if (pr.answer) {
...
/* **bold** 와 줄바꿈을 HTML로. XSS 방지 위해 먼저 이스케이프. */
...
function renderNavFooter() {
  ...
  next.disabled = app.currentIdx >= app.flatSections.length - 1;
  prog.textContent = `${app.currentIdx + 1} / ${app.flatSections.length}`;
...
function bindKeyboard() {
  document.addEventListener("keydown", e => {
    ...
    if (e.key === "ArrowRight" || e.key === "PageDown") {
      ...
      // 현재 뷰포트에서 보이는 첫 답 토글
      if (e.target.tagName === "BUTTON") return; // 버튼 포커스 시엔 기본 동작
      e.preventDefault();
